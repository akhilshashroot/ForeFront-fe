...
const ShiftConfigCreate = (props) => {
  ...
  const [shiftTable, setShiftTable] = useState(true);
  ...
  console.log("🚀 ~ file: ShiftConfigCreate.js ~ line 101 ~ handleChange ~ mondayData", mondayData)

  const handleChange = (e, val, day) => {
  console.log("🚀 ~ file: ShiftConfigCreate.js ~ line 99 ~ handleChange ~ e, val, day", e, val, day)
      ...
      if (day == "Monday") {
        ...
        if (preData !== -1) {
          ...
          let newData = {
            day: day,
            val: val,
            member: val === "comment" ? e : e!==null ? e.map(m=>m.label).toString():'-',
          };
          copy[preData] = newData;
          setMondayData([...copy]);
        } else {
          let newData = [
            { day: day, val: val, member: val === "comment" ? e : e!==null ? e.map(m=>m.label).toString():'-' },
          ];
        ...
        if (preData !== -1) {
          ...
          let newData = { day: day, val: val, member: val === "comment" ? e : e!==null ? e.map(m=>m.label).toString():'-'};
          copy[preData] = newData;
          setTuesdayData([...copy]);
        } else {
          let newData = [{ day: day, val: val, member: val === "comment" ? e : e!==null ? e.map(m=>m.label).toString():'-'}];
    ...
    <>
          ...
          {!shiftTable && (
            <PreviousShiftTable
              data={
                props.shift &&
                props.shift.getPreviousShift &&
                props.shift.getPreviousShift.data
              }
            />
          )}
