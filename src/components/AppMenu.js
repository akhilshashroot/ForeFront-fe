...
if (isUserAuthenticated() && getLoggedInUser().role=="Admin") {            
    Childmenu = item.children.filter((t) => t.role_number.includes(getLoggedInUser().role_number));   
}
...
class AppMenu extends Component<Props> {
    ...
    render() {
        ...
        if (isUserAuthenticated() && getLoggedInUser().role) {   
            
            if(getLoggedInUser().role == 'Admin'){
                let newMenuItems1 = menuItems.filter((t) => t.roles.includes(getLoggedInUser().role));
                 newMenuItems = newMenuItems1.filter((t) => t.role_number.includes(getLoggedInUser().role_number));
