...
const ProfileMenus = [
    {
        label: 'My Account',
        icon: 'uil uil-user',
        redirectTo: getLoggedInUser().role === 'Admin' ? '/admin/dashboard' : '/employee/dashboard',
    },
    {
        label: 'Settings',
        icon: 'uil uil-cog',
        redirectTo: getLoggedInUser().role === 'Admin' ? '/admin/settings' : '/employee/userprofile',
    },
    // {
    //   label: "Support",
    ...
    render() {
        ...
        return (
            <React.Fragment>
                ...
                <button className="nav-link dropdown-toggle arrow-none btn btn-link">
                  {/* <li>
                    {user.role === "Admin" ? (
                      <span onClick={this.toggle1}
                        // to="/"
                        className="mdi mdi-settings-outline noti-icon"
                      ></span>
                    ) : (
                      <Link
                        to="/userprofile"
                ...
                <NotificationDropdown notifications={Notifications} />
                            ...
                            <li className="notification-list">
                                <ProfileDropdown
                                    profilePic={getLoggedInUser().profile_pic}
                        ...
                        {this.state.modal && (
                            <UserProfile toggle={this.toggle} toggle1={this.toggle1} modal={this.state.modal} />
                        )}
                    </div>
                </div>
            </React.Fragment>
